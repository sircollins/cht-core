const cb_request = require('../../src/couch-request');
...
class Unicorn { }
const notPlainObjects = [[1, 2, 3], new Unicorn(), new Map([[1, 1], [2, 2], [3, 3]]), new Set([1, 2, 3]), () => { }, true, 'foo', null, 1, NaN, Infinity, /foo/, new Date(), new Error(), new Int8Array(), new Float32Array(), new Float64Array(), new Uint8Array(), new Uint8ClampedArray(), new Uint16Array(), new Uint32Array(), new ArrayBuffer(), new WeakMap(), new WeakSet()];
  ...
  notPlainObjects.forEach(notPlainObject => {
    if (notPlainObject == null) {
      it(`Rejects (method: get): (notPlainObject == null)`, done => {
        chai.expect(cb_request.get(notPlainObject)).to.eventually.be.rejectedWith(optionsErrorMsg).and.be.an.instanceOf(Error).notify(done);
      });
      it(`Rejects (method: post): (notPlainObject == null)`, done => {
        chai.expect(cb_request.post(notPlainObject)).to.eventually.be.rejectedWith(optionsErrorMsg).and.be.an.instanceOf(Error).notify(done);
      });
      it(`Rejects (method: put): (notPlainObject == null)`, done => {
        chai.expect(cb_request.put(notPlainObject)).to.eventually.be.rejectedWith(optionsErrorMsg).and.be.an.instanceOf(Error).notify(done);
      });
      it(`Rejects (method: delete): (notPlainObject == null)`, done => {
        chai.expect(cb_request.delete(notPlainObject)).to.eventually.be.rejectedWith(optionsErrorMsg).and.be.an.instanceOf(Error).notify(done);
      });
      it(`Rejects (method: head): (notPlainObject == null)`, done => {
        chai.expect(cb_request.head(notPlainObject)).to.eventually.be.rejectedWith(optionsErrorMsg).and.be.an.instanceOf(Error).notify(done);
      });
    } else {
      let toString = `${notPlainObject.toString()}`;
      let result = toString === "" ? Object.getPrototypeOf(notPlainObject).constructor.name : toString;
...
describe('Couch request receives correct options and returns stub value', () => {
  ...
  beforeEach(() => {
    sinon.stub(environment, 'host').value('test.com');
    requestGet = sinon.stub(request, 'get');
    requestPost = sinon.stub(request, 'post');
    requestPut = sinon.stub(request, 'put');
    requestDelete = sinon.stub(request, 'delete');
  ...
  options.forEach(option => {
    it(`Get: Called with composite options and returns "foo"`, async function () {
      const result = await cb_request.get(option)
    ...
    it(`Post: Called with composite options and returns "foo"`, async function () {
      const result = await cb_request.post(option)
    ...
    it(`Head: Called with composite options and returns "foo"`, async function () {
      const result = await cb_request.head(option)
    ...
    it(`Delete: Called with composite options and returns "foo"`, async function () {
      const result = await cb_request.delete(option)
    ...
    it(`Put: Called with composite options and returns "foo"`, async function () {
      const result = await cb_request.put(option)
